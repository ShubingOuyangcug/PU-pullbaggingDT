...
def AUCC(alldata_sort,tnsdata_sort,side,flag):
    ...
    for iii in range(1, 101):
        fflag = 100 - iii
        re3 = np.where(np.logical_and(re2 <= (100 / 100), re2 >= (fflag / 100)))# 举例从百分100到百分99有几个，100到98有几个，。。。。
        nnum = nnum + len(re3[0]) #算的是累积面积，看公式
...
ospath = r'../result_picture/10yinzi' #，he，pubagging，RF，svm，AdaBoost2，XGBOOST2,,lr,,zuotu,yuanlizuotuguiyi
for i in os.listdir(ospath):
    if "tif" in i:
        ...
        path = r'../result_picture/pubagging/1000puba4.tif'
        # print(path)
        X1 = read(path)
        BILI=1
